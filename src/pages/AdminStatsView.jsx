...
const AdminStatsView = () => {
  ...
  const OverviewTab = () => {
    // Cálculo de datos acumulados para las ojivas
    const monthlySalesData = completeStats?.monthlySales || [];
    let cumulativeRevenue = 0;
    const cumulativeMonthlySales = monthlySalesData.map(sale => {
      cumulativeRevenue += sale.totalSales;
      return { ...sale, cumulativeSales: cumulativeRevenue };
    });

    const topProductsData = completeStats?.topProductsLast30Days || [];
    let cumulativeProductSales = 0;
    const cumulativeTopProducts = topProductsData.map(product => {
      cumulativeProductSales += product.totalQuantity;
      return { ...product, cumulativeQuantity: cumulativeProductSales };
    });
    ...
    return (
      <div className="d-grid gap-4">
        ...
        {/* Gráfico de Ventas Mensuales (Ojiva) */}
        {cumulativeMonthlySales.length > 0 ? (
          <div className="stats-card-minimal">
            <h3 className="card-title fs-5 fw-bold mb-4 text-dark">Ojiva de Ventas Acumuladas Mensuales</h3>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={cumulativeMonthlySales}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="month" label={{ value: "Meses", position: "insideBottom", offset: -5 }} />
                <YAxis tickFormatter={(value) => `$${value}`} label={{ value: 'Ventas Acumuladas (USD)', angle: -90, position: "insideLeft" }} />
                <Tooltip formatter={(value) => [formatCurrency(value), 'Ventas Acumuladas']} />
                <Legend />
                <Line type="monotone" dataKey="cumulativeSales" stroke="#FF00FF" strokeWidth={3} name="Ventas Acumuladas" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        ) : (
        ...
        {/* Top productos vendidos (Ojiva) */}
        {cumulativeTopProducts.length > 0 ? (
          <div className="stats-card-minimal">
            <h3 className="card-title fs-5 fw-bold mb-4 text-dark">Ojiva de Productos Vendidos (Últimos 30 Días)</h3>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={cumulativeTopProducts}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="productName" angle={-45} textAnchor="end" height={100} />
                <YAxis label={{ value: 'Unidades Vendidas Acumuladas', angle: -90, position: "insideLeft" }} />
                <Tooltip formatter={(value) => [value, 'Cantidad Acumulada']} />
                <Legend />
                <Line type="monotone" dataKey="cumulativeQuantity" stroke="#00C49F" name="Cantidad Vendida Acumulada" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        ) : (
  ...
  const CollaboratorsTab = () => {
    // Cálculo de datos acumulados para la ojiva de colaboradores
    const collaboratorSalesData = collaboratorSales.sort((a, b) => b.totalSales - a.totalSales);
    let cumulativeSales = 0;
    const cumulativeCollaboratorSales = collaboratorSalesData.map(c => {
      cumulativeSales += c.totalSales;
      return { ...c, cumulativeSales };
    });
    ...
    return (
      <div className="stats-card-minimal">
        ...
        {/* Gráfico de ojiva de colaboradores */}
        <div className="mb-4">
          {cumulativeCollaboratorSales && cumulativeCollaboratorSales.length > 0 ? (
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={cumulativeCollaboratorSales.slice(0, 10)}>
                <CartesianGrid strokeDasharray="3 3" />
